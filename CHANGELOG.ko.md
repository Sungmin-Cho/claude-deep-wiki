...
- **`skills/wiki-ingest/SKILL.md` Step 11 — `index.md` dashboard 재정의.** 기존 명세는 매 ingest 마다 LLM 으로 전체 페이지 catalog 재작성을 요구 — 100 페이지 이상에서 50K+ output tokens 가 필요해 실제로는 SKIP 되어 stale 상태로 유지됨. v1.7.0 부터 `index.md` 는 가벼운 dashboard 로 재정의: wiki 1-paragraph overview, At a glance stats (pages/tags/last ingest/last catalog refresh), Recent activity (`log.jsonl` 의 `pages_created`/`pages_updated` array 를 expand 해 최근 7일치 top 10), Top tags (top 15 + 1-sentence description), Featured pages (frontmatter `featured: true` opt-in, lex-sort, cap 30, code-derived first-sentence summary). dashboard 의 첫 줄은 HTML 주석 marker `<!-- deep-wiki-dashboard-v1.7.0 -->` 로 시작 — backup guard 가 v1.7.0+ dashboard 와 legacy catalog 를 명확히 구분 (손수 작성한 pre-1.7.0 catalog 가 우연히 구조적 패턴 매칭 시 backup skip 되는 위험 제거). 전체 machine-readable catalog 는 그대로 `.wiki-meta/index.json` 에, 시계열 기록은 `log.jsonl` 에 남는다. ingest 당 token 예산 ~3-5 KB input / ~3-5 KB output 으로 page 수와 무관하게 안정. marker 가 없는 첫 1.7.0 ingest 시 pre-existing `index.md` 를 `<wiki_root>/.wiki-meta/.backups/index.md.pre-1.7.0` 으로 자동 backup (idempotent) — 손수 편집한 사용자 catalog 보호. `.backups/` 는 v1.7.0 에서 신설된 디렉토리로 `.versions/` 의 wiki-lint `excess_versions` auto-fix (keep: 3 pruning) 영향을 받지 않음.
- **`skills/wiki-setup/SKILL.md` — fresh wiki 가 v1.7.0 dashboard 형식으로 seed.** Setup 이 이제 `<!-- deep-wiki-dashboard-v1.7.0 -->` marker + 5-section sparse scaffold ("newly created" placeholder) 으로 `index.md` 작성. 이전 setup 은 `# Wiki Index` / `## Pages` legacy catalog 를 만들었기 때문에 첫 ingest 가 그것을 손수 작성된 legacy 로 인식해 불필요한 backup 발생.
...
- **`skills/wiki-ingest/SKILL.md` Step 0.5 — inbox stale cleanup + `partial_fail` 보호 + quarantine-not-delete 정책.** Prerequisites 블록 (`WIKI_ROOT` 설정) 과 Step 1 (Identify Sources) 사이에 삽입. wiki lock 미보유 상태에서 실행 — 안전성은 7일 mtime threshold + `<wiki_root>/.wiki-meta/sources/*.yaml` 의 미해결 `partial_fail:` sentinel 참조 파일 명시적 제외로 확보. Stale 파일은 `rm -f` 가 아니라 `<wiki_root>/.wiki-meta/.inbox/.quarantine/<basename>.<epoch>` 으로 **이동** — Step 6.5 → Step 7.6.F race window 에서 partial_fail sentinel 작성 전 crash 시 사용자의 pasted text 가 영구 손실되지 않도록 recovery surface 보존 (Codex adversarial round 3). Step 12 의 "self-session files only, no wildcard" 정책은 그대로 유지. partial_fail 보호 스캔의 `origin:` parser 는 double-quoted 와 single-quoted yaml 두 형식 모두 수용 (Step 1.5 origin parser 와 mirror — round 3 latent regression 수정). Bash 3.2 portable: GNU `stat -c %Y` 우선 + BSD `stat -f %m` fallback + numeric guard (Linux 의 `stat -f` 는 filesystem info 를 exit 0 으로 반환하므로 단순 `||` 체인은 깨짐).
...
- **`tests/inbox-cleanup.test.js`** — F3 회귀 테스트 4종: 8일 이상 파일 quarantine (삭제 아님, 내용 보존 + epoch-suffixed) / 1일 이내 파일 in-place 보존 (.quarantine/ 비어있음) / double-quoted `origin:` 의 `partial_fail` 참조 파일 보존 / single-quoted `origin:` 의 `partial_fail` 참조 파일 보존 (broadened sed pattern 회귀 커버). bash body 는 `execSync('bash', { input, env: { WIKI_ROOT } })` 로 stdin + env 전달 (outer-shell expansion 회피).
...
- `npm test` 통과 — 131 tests (126 pre-change + F3 4종 + F1 1종, bare-discovery 자동 포함).
...
- patch release 로 package/plugin manifest 버전을 1.6.1 → 1.6.2 로 동기화.
