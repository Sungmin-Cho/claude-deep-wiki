...
- `/wiki-ingest` F1 all-dropped 3-strike의 터미널 `ingest-fail` 로그 emit을 가드 승격이 scan window를 durable하게 전진/해제한 **뒤**로 옮겼습니다. 기존에는 먼저 emit되어, rename 실패(`.pending-scan`+retry 카운터 보존 후 bail — R2 fix) 시에도 append-only 로그에 터미널 `ingest-fail` 행이 남았고, 다음 재시도 사이클이 또 emit해 감사 로그가 실제 scan-window 상태와 불일치했습니다. 이제 emit은 승격 성공 후에만 실행되며(rename 실패 경로는 그 앞에서 return), 실패한 escape는 durable한 터미널 행을 남기지 않습니다.
...
- 터미널 로그-순서 fix를 sibling `/wiki-ingest` Step 7.7.B all-workers-fail 3-strike 경로에 미러했습니다. 이 경로도 여전히 `ingest-fail`을 가드 승격 앞에서 emit해 F1이 고친 것과 동일한 실패 모드(rename 실패 시 터미널 행 중복 + 감사 불일치)를 가졌습니다. 이제 emit은 승격이 window를 durable하게 전진/해제한 뒤에만 실행되며, 가드 승격은 그 앞에서 rename 실패 시 bail하므로 실패한 escape는 터미널 행을 남기지 않습니다. 모든 `ingest-fail` emit 지점 grep으로 이 둘이 유일함을 확인했습니다.
