...
- Moved the `/wiki-ingest` F1 all-dropped 3-strike terminal `ingest-fail` log emit to *after* the guarded promotion durably advances or releases the scan window. It was emitted first, so a rename failure — which preserves `.pending-scan` + the retry counter and bails (the R2 fix) — still left a terminal `ingest-fail` row in the append-only log; the next retry cycle then emitted another and the audit log disagreed with the real scan-window state. The emit now runs only after the promotion succeeds (the rename-failure path returns before it), so a failed escape leaves no durable terminal row.
...
- Mirrored the terminal-log-order fix onto the sibling `/wiki-ingest` Step 7.7.B all-workers-fail 3-strike path, which still emitted the `ingest-fail` log line before the guarded promotion (the same duplicate-terminal-row / audit-mismatch failure mode on a rename failure that the F1 path fixed). The emit now runs only after the promotion durably advances or releases the window; the guarded promotion bails on a rename failure before it, so a failed escape leaves no terminal row. A grep of every `ingest-fail` emit site confirms these are the only two.
