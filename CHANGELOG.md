...
- Completed scan-window journals no longer accumulate without bound. Automatic maintenance retires old validated `ensure` terminals under the current lock, and the bounded `transaction prune` command removes only age-eligible, fully validated `cleaned` scan-window directories while preserving in-flight, malformed, foreign-kind, linked, or ambiguous state.
