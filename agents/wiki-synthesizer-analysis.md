...
4. For each `update` decision, **capturing the FULL VERBATIM bytes of `existing_page_body`** — the entire markdown file content (frontmatter + body + trailing newline), NOT a summary, intro excerpt, or context-window-sized slice. Use the `Read` tool with NO `offset`/`limit` parameters; the Read tool returns full file content by default for typical wiki pages (median ~5–10KB; up to ~2000 lines without explicit limit). These bytes flow into Stage 2 workers as synthesis context AND were the legacy v1.4.0/v1.4.1 source for main's C3 hash baseline. **v1.4.2+ main reads from disk independently for hash baseline (F1 fix — defense-in-depth recovery against synthesizer truncation; see Field semantics for `existing_body_hash` below); spec-compliant agent emit still MUST carry the full bytes** so Stage 2 worker synthesis is coherent with the page state Stage 1 analyzed. Emit `existing_body_hash: "main-computes"` (sentinel — see "Source bytes hash semantic drift" + below).
...
- `existing_page_body` — **FULL VERBATIM bytes** of the existing page (frontmatter + body + trailing newline), captured during Phase 1b/1c read. `null` for `create`. Stage 2 workers receive this verbatim as synthesis context. Do NOT truncate, summarize, or emit only an intro excerpt — Stage 2 worker merge logic depends on seeing the full prior page so unrelated sections survive (Rule 5 + `preserve_sections`). **v1.4.2+: main also re-reads from disk for the C3 hash baseline (F1 fix — defense-in-depth against synthesizer truncation); see `existing_body_hash` semantic below.**
- `existing_body_hash` — sentinel string `"main-computes"` for `update` entries (Stage 1 cannot shasum); `null` for `create`. **v1.4.2+: main computes sha256 from DISK-AUTHORITATIVE bytes** (re-reads `<wiki>/pages/<file>` in `commands/wiki-ingest.md` Step 7.5 single-source branch) rather than from the `existing_page_body` emitted here. The disk-read is the canonical input to the C3 concurrency check at Step 7.6.C — that closes the v1.4.1 dogfood F1 truncation false-positive (Stage 1 emit drift abandoning every update via spurious "concurrent ingest" abort). The synthesizer's `existing_page_body` flows to Stage 2 workers as synthesis context; main's hash baseline is computed independently from disk under no-write window between Stage 1 return and Stage 3 lock. Pre-v1.4.2 contract (main computes from `existing_page_body`) is preserved as a fallback when main detects a synthesizer-emit / disk-bytes match within EOL tolerance, ensuring spec-compliant agents see byte-identical hashing semantics.
...
- **`existing_body_hash` for update entries:** Stage 1 emits the sentinel string `"main-computes"` (synthesizer tool whitelist excludes shasum/Bash). **v1.4.2+: main computes sha256 from DISK-AUTHORITATIVE bytes** — re-reads `<wiki>/pages/<file>` in `commands/wiki-ingest.md` Step 7.5 single-source branch IMMEDIATELY after parsing analysis output, BEFORE entering Stage 3, and stores the disk-read hex hash for downstream C3 concurrency check. The synthesizer's `existing_page_body` flows to Stage 2 workers (or to the inline-write path) as synthesis context; main's hash baseline is independent of synthesizer's bytes (closes v1.4.1 dogfood F1 truncation gap). Stage 3 main re-reads page body under lock and compares against the main-computed disk hash to detect concurrent ingest commits (mandatory check; see `commands/wiki-ingest.md` Step 7.6 C3 concurrency guard). Pre-v1.4.2 spec text (hash from `existing_page_body`) was vulnerable to LLM truncation drift; the v1.4.2 disk-authoritative contract preserves byte-identical Stage 3 behavior for spec-compliant agents (whose emit matches disk within EOL tolerance) while neutralizing the F1 false-positive abort path.
