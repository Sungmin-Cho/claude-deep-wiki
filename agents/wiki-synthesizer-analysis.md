...
4. For each `update` decision, **capturing the FULL VERBATIM bytes of `existing_page_body`** — the entire markdown file content (frontmatter + body + trailing newline), NOT a summary, intro excerpt, or context-window-sized slice. Use the `Read` tool with NO `offset`/`limit` parameters; the Read tool returns full file content by default for typical wiki pages (median ~5–10KB; up to ~2000 lines without explicit limit). **v1.4.2+ contract:** main consumes these bytes ONLY for telemetry comparison (F1 disk-authoritative gate computes the size delta against main's own disk read and emits a stderr WARN when the band is exceeded). Main does NOT use the synthesizer's emit for the C3 hash baseline OR for Stage 2 worker / inline-write synthesis context — both are sourced from main's authoritative disk read in `skills/wiki-ingest/SKILL.md` Step 7.5 single-source branch. Spec-compliant agents must still emit full bytes for forward-compat (a future v1.5+ revision may restore synthesizer-bytes flow once truncation drift is empirically resolved at the LLM layer) and for audit honesty. Emit `existing_body_hash: "main-computes"` (sentinel — see "Source bytes hash semantic drift" + below).
...
- `existing_page_body` — **FULL VERBATIM bytes** of the existing page (frontmatter + body + trailing newline), captured during Phase 1b/1c read. `null` for `create`. **v1.4.2+: not consumed for downstream synthesis context.** Pre-v1.4.2 (v1.4.0–v1.4.1) Stage 2 workers received this verbatim as synthesis context. v1.4.2 main's F1 disk-authoritative gate replaces this field with main's own disk read before Stage 2 dispatch / inline-write — closing the truncation drift channel that caused the v1.4.1 dogfood C3 false-positive. The agent's emit is consumed for the F1 size-delta WARN telemetry only. Spec-compliant emits (full verbatim bytes) keep this telemetry quiet and preserve forward-compat with a future v1.5+ revision that may restore synthesizer-bytes flow.
- `existing_body_hash` — sentinel string `"main-computes"` for `update` entries (Stage 1 cannot shasum); `null` for `create`. **v1.4.2+: main computes sha256 from DISK-AUTHORITATIVE bytes** (re-reads `<wiki>/pages/<file>` in `skills/wiki-ingest/SKILL.md` Step 7.5 single-source branch) rather than from the `existing_page_body` emitted here. The disk-read is the canonical input to the C3 concurrency check at Step 7.6.C — that closes the v1.4.1 dogfood F1 truncation false-positive (Stage 1 emit drift abandoning every update via spurious "concurrent ingest" abort). Hashing is consistent within Stage 3: both F1 capture and C3 re-check use `$(cat …)` byte-stripping (POSIX command substitution drops trailing newlines), so concurrent-ingest detection between F1 capture and Stage 3 lock acquire still fires on disk byte changes. Hash values are NOT byte-identical to v1.4.1 (v1.4.1 hashed `printf '%s' "$emit"` which preserved synthesizer's trailing newline if compliant; v1.4.2 hashes `$(cat)` output which strips them). Stage 3 success/abort decisions remain equivalent for spec-compliant agents — the byte-strip is symmetric across both sides of the C3 comparison.
...
- **`existing_body_hash` for update entries:** Stage 1 emits the sentinel string `"main-computes"` (synthesizer tool whitelist excludes shasum/Bash). **v1.4.2+: main computes sha256 from DISK-AUTHORITATIVE bytes** — re-reads `<wiki>/pages/<file>` in `skills/wiki-ingest/SKILL.md` Step 7.5 single-source branch IMMEDIATELY after parsing analysis output, BEFORE entering Stage 3, and stores the disk-read hex hash for downstream C3 concurrency check. The synthesizer's `existing_page_body` is NO LONGER consumed for Stage 2 worker / inline-write context (v1.4.2 fix F1.5 — agent doc contract correction). Main re-reads disk and substitutes those bytes into the per-entry `existing_page_body` field before downstream dispatch. Stage 3 main re-reads page body under lock and compares against the main-computed disk hash to detect concurrent ingest commits (mandatory check; see `skills/wiki-ingest/SKILL.md` Step 7.6 C3 concurrency guard). Pre-v1.4.2 spec text (hash from `existing_page_body`) was vulnerable to LLM truncation drift. The v1.4.2 disk-authoritative contract neutralizes the F1 false-positive abort path while keeping concurrent-ingest detection scope intact (the `$(cat …)` newline-strip is symmetric across F1 capture and Stage 3 C3 re-check); hash values are NOT byte-identical to v1.4.1 due to command-substitution newline normalization, but Stage 3 decisions are equivalent for spec-compliant agents.

> **Trust boundary acknowledgement (M1 — v1.4.1 Track C closure; v1.4.2 F2 bracketing extension):**
...
>
> Caller substitution (e.g., main session voluntarily downgrading to `subagent_type: "general-purpose"`) is the residual risk — addressed by V-0 caller-side resolution probe (per plan §3.3) and the `_post_dispatch_dirty_scan()` guard. **v1.4.2 F2 fix extends §3.9 bracketing to this Stage 1 dispatch** (label `"A5-analysis"`); the `_post_dispatch_pre_snapshot` runs immediately before `invoke deep-wiki:wiki-synthesizer-analysis` and the post-scan immediately after Stage 1 returns (before page_plan branch decision in `skills/wiki-ingest/SKILL.md` Step 7.5 single-source branch). This catches a downgraded analysis subagent's `<wiki_root>/`-internal mutations regardless of downstream sub-threshold vs. A5 fanout branching. L2 limitation (off-root writes such as `/tmp/`) still applies — see CHANGELOG v1.4.1 §11.5 for honest disclosure of dirty-scan scope.
...
Worker mode (`wiki-synthesizer-worker`) preserves `proposed_action: "skip"` for the multi-source A4 path. Analysis mode has NO skip — Stage 1 simply does not emit a `page_plan` entry for unaffected candidates. This matches v1.3.0 inline-mode behavior of just-not-producing-a-draft. If the analysis judges the source contributes nothing new to any candidate, return `page_plan: []` + `inline_bodies: []`; the caller treats this as `ingest-skip` terminal event (Step 7.8 in `skills/wiki-ingest/SKILL.md`).
