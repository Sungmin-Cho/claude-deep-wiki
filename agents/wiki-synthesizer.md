...
- `a5_fanout_threshold` (v1.4.0+, optional, only used by `mode: "analysis"`) — integer threshold below which analysis mode emits `inline_bodies` alongside `page_plan` (no Stage 2 fanout needed). When `len(page_plan) < a5_fanout_threshold`, generate `page_content` for each entry inline within the same LLM context and emit `inline_bodies`. When `len(page_plan) >= a5_fanout_threshold`, emit `inline_bodies: []` (caller will dispatch one `wiki-page-writer` worker per entry). Default when omitted: `3`. Caller (main) reads its `.config.json` value into `$A5_FANOUT_THRESHOLD` and passes it here so the synthesizer's threshold decision matches main's downstream branching. Without this pass-through, user-set overrides (e.g. threshold=9999 to disable fanout) would be ignored by the agent and main's sub-threshold branch would later abort on lex-set mismatch (post-review fix — fixes round-4 missed: W1).
...
6. **If `len(page_plan) < a5_fanout_threshold`**, ALSO generate the page_content for each entry inside the same LLM context (no extra invocation), and emit `inline_bodies` array. Otherwise `inline_bodies = []`. **Read `a5_fanout_threshold` from the input contract (default 3 if omitted)** — this is the user-tunable knob from `<wiki>/.wiki-meta/.config.json`, passed through by main. Do NOT use a hardcoded constant; main's downstream branching (sub-threshold vs A5 fanout) uses the same value (post-review fix — fixes round-4 missed: W1).
