 ...
 *   - producer === 'deep-wiki' (plugin identity).
 ...
 *   - provenance.source_artifacts[].path non-empty, run_id (if present) is a
 *     string (suite envelope schema does not impose length/format on
 *     run_id within source_artifacts — chain-extraction helpers like
 *     wrap-index-envelope.js#tryReadEnvelopeRunId enforce ULID format on the
 *     trusted run_id field. External producers MAY use non-ULID conventions.
...
const ALLOWED_KINDS = new Set(['index']);
