...
3. **Lock atomicity** — All write operations MUST acquire `mkdir <wiki>/.wiki-meta/.wiki-lock` before touching any wiki state, and MUST release it before the end of their critical section. `mkdir` is the single atomic primitive that guarantees mutual exclusion across all filesystems. The **release mechanism** (unconditional EXIT trap / failure-only trap + explicit success release / contention soft-fail) depends on whether the lock spans one or many Bash blocks — follow the trap pattern catalog in `references/storage-layout.md` §Concurrency Lock Protocol rather than hard-coding a single trap form (a literal `trap 'rmdir' EXIT` in a multi-block skill releases the lock early). Enforced at: `skills/wiki-ingest/SKILL.md` (Step 7.6.C / Step 7.5.M-C), `skills/wiki-rebuild/SKILL.md` Step 1, `skills/wiki-query/SKILL.md` (Layer 2), `skills/wiki-lint/SKILL.md` (§13 Auto-Fix / Phase A). See `references/storage-layout.md` for the full protocol + stale-lock recovery.
